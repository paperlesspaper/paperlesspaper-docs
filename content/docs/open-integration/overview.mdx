...
Use [Webhooks & content push](/open-integration/webhooks) when an external event should update a paper, such as a chat message or a home automation. Your backend connects through a short-lived grant, pushes text or images, and receives authenticated delivery status callbacks. The render page receives that source as `payload.integrationContent` and owns its layout.

Content belongs to the saved paper and is shared by all frames displaying it. The latest message replaces the previous source; device updates still depend on each frame's wake schedule. See [Webhook & API reference](/open-integration/webhook-reference) for endpoints, states, retries, and troubleshooting.
