---
title: Open Integration Settings Pages
...
The current public reference implementation lives in the `open-integration-example` inside `paperlesspaper-apps`.
...
The public example also supports older message shapes. That matters if you want compatibility with older hosts or renderers.

Legacy host to plugin messages:
...
Legacy plugin to host messages:
...
If you are starting from scratch, implement the structured protocol first, then add legacy support only if you need it.
