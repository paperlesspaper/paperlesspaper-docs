...
Follow [Webhooks & content push](/open-integration/webhooks) for the complete setup and [Webhook & API reference](/open-integration/webhook-reference) for status callbacks and revocation.
...
For the full provider model, manifest, and render-page contract, go back to [Open Integrations](/open-integration/overview).
